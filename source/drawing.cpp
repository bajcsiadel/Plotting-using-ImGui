...
struct WindowDraw : virtual public DrawBase
{
    public: 
        ...
        WindowDraw(ImDrawList* f, ImVec2 pos, ImVec2 prop) : frame(f) {
            draw_pos   = pos;
            proportion = prop;
...
int init_window()
{
    ...
    global.video.height = global.video.width = global.video.width + (global.video.width % 2 == 0 ? 0 : 1);
...
void init_settings_window(bool *show)
{
    ...
    ImGui::Begin("Settings", show,
        ...
        if (ImGui::CollapsingHeader("Movie"))
        {
            ...
            {
                ImGui::Text("Zoomed area: %.2f x %.2f", global.movie.zoom.width, global.movie.zoom.height);
                ImGui::Text("Left upper corner: (%.2f, %.2f)", global.movie.zoom.corners[0].x, global.movie.zoom.corners[0].y);
                ImGui::Text("Right lower corner: (%.2f, %.2f)", global.movie.zoom.corners[1].x, global.movie.zoom.corners[1].y);
                ImGui::TreePop();
                ImGui::Separator();
            }
